...
export default function Demo2Intro({ onComplete }: Props) {
  const { profile } = useDemo2();
  const { t } = useLanguage();
  ...
  return (
    <div className="fixed inset-0 z-50 bg-foreground overflow-hidden flex items-center justify-center">
      <AnimatePresence mode="wait">
        {phase === "coin-reveal" && (
          <motion.div key="coin-reveal" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1.5 }}
            ...
            <div className="mt-12 h-20 flex items-center justify-center">
              <AnimatePresence mode="wait">
                {currentSubtitle && (
                  <motion.p key={currentSubtitle} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
        ...
        {(phase === "coin-translate" || phase === "text-appear" || phase === "mode-choice") && (
          <motion.div key="main-layout" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}
            ...
            <LayoutGroup>
              <motion.div layout transition={{ layout: { duration: 0.7, ease: [0.22, 1, 0.36, 1] } }}
                ...
                {phase === "mode-choice" && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8, delay: 0.3 }}
                    ...
                    <button onClick={() => onComplete("voice")} className="group flex flex-col items-center gap-3 transition-all">
                    ...
                    <button onClick={() => onComplete("text")} className="group flex flex-col items-center gap-3 transition-all">
                      <div className="w-20 h-20 rounded-full border border-background/10 flex items-center justify-center group-hover:border-background/30 group-hover:bg-background/[0.03] transition-all duration-300">
