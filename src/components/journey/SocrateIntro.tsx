      ...
      <AnimatePresence mode="wait">
        ...
        {(phase === "coin-translate" || phase === "text-appear" || phase === "mode-choice") && (
          <motion.div key="main-layout" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }} className="fixed inset-0 flex items-center justify-center">
            <div className="flex items-center gap-12 max-w-3xl px-8">
              {/* Text column */}
              ...
              {/* Coin to the right, vertically centered with text */}
              <motion.div initial={{ x: 0, y: 0, scale: 1 }} animate={{ scale: 0.45 }} transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1] }} className="flex-shrink-0">
                <SocrateCoin size={280} interactive={false} />
              </motion.div>
            </div>
          </motion.div>
