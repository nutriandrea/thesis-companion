...
type ContactType = { type: "supervisor" | "expert"; name: string; title: string; email: string; org: string; fieldIds: string[]; about: string | null; };
...
export default function ContactsPage() {
  ...
  const filtered = useMemo(() => {
    return allContacts
      ...
      .sort((a, b) => {
        const aMatch = a.fieldIds.filter(f => currentStudent.fieldIds.includes(f)).length;
        const bMatch = b.fieldIds.filter(f => currentStudent.fieldIds.includes(f)).length;
        return bMatch - aMatch;
      });
  }, [search, filter, allContacts, currentStudent.fieldIds]);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">{filtered.length} contatti disponibili</p>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cerca per nome o organizzazione..."
        ...
        {filtered.slice(0, 20).map((contact, i) => (
          <motion.div key={contact.email} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
            ...
            <div className={`w-11 h-11 rounded-full flex items-center justify-center shrink-0 ${
              contact.type === "supervisor" ? "bg-accent/10 text-accent" : "bg-ai/10 text-ai"
            }`}>
