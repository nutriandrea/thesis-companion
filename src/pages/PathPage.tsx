...
import { Route, GraduationCap, Building2, Users } from "lucide-react";
...
interface PersonalizedPath { topic: Topic; supervisor: Supervisor; company: Company | null; experts: Expert[]; matchScore: number; steps: string[]; }
...
export default function PathPage() {
  ...
  const paths = useMemo<PersonalizedPath[]>(() => {
    return topics
      ...
      .slice(0, 4)
      .map(topic => {
        const sup = supervisors.find(s => s.fieldIds.some(f => topic.fieldIds.includes(f))) || supervisors[0];
        ...
        return {
          topic, supervisor: sup, company, experts: topicExperts,
          matchScore: Math.min(95, 60 + fieldOverlap * 15),
          steps: [
            `Contatta ${sup.title} ${sup.lastName}`,
            company ? `Collaborazione con ${company.name}` : "Cerca partner industriale",
            "Literature review iniziale",
            "Definisci metodologia",
            topicExperts[0] ? `Incontro con ${topicExperts[0].firstName} ${topicExperts[0].lastName}` : "Cerca mentori",
            "Fase sperimentale",
          ],
        };
      })
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {paths.map((path, i) => (
          <motion.div key={path.topic.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
              <div className="flex items-center gap-2 p-2.5 rounded-lg bg-accent/5 border border-accent/10">
                ...
                <div className="min-w-0"><p className="text-xs text-muted-foreground">Supervisore</p><p className="text-sm font-medium truncate">{path.supervisor.title} {path.supervisor.lastName}</p></div>
              </div>
              {path.company && <div className="flex items-center gap-2 p-2.5 rounded-lg bg-success/5 border border-success/10"><Building2 className="w-4 h-4 text-success shrink-0" /><div className="min-w-0"><p className="text-xs text-muted-foreground">Azienda</p><p className="text-sm font-medium truncate">{path.company.name}</p></div></div>}
              {path.experts[0] && <div className="flex items-center gap-2 p-2.5 rounded-lg bg-ai/5 border border-ai/10"><Users className="w-4 h-4 text-ai shrink-0" /><div className="min-w-0"><p className="text-xs text-muted-foreground">Esperto</p><p className="text-sm font-medium truncate">{path.experts[0].firstName} {path.experts[0].lastName}</p></div></div>}
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 mt-4 pt-3 border-t">
              {path.topic.fieldIds.map(f => <Badge key={f} variant={studentFields.includes(f) ? "default" : "secondary"} className="text-xs">{getFieldName(f)}</Badge>)}
            </div>
          </motion.div>
        ))}
      </div>
